...
const DashboardUser = () => {
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (parsedRoles.length > 0) {
          ...
          if (roles.some((role: { role_name: string }) => role.role_name === 'USER')) {
            newRoles = ['USER'];
        ...
        // Force default to USER (or CUSTOMER)
        if (!newRoles.includes('USER') && !newRoles.includes('CUSTOMER')) {
          newRoles = ['USER'];
          localStorage.setItem('selectedRoles', JSON.stringify(newRoles));
          localStorage.setItem('selectedRole', 'USER');
        }
  ...
  return (
    <Fragment>
      <Container>
        <Toolbar>
          <ToolbarHeading title="User Dashboard" description="Central Hub for User View" />
          <ToolbarActions>
            {isDropdownReadonly ? (
